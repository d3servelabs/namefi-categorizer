...
import {
  extractChildDomain, extractParentDomains, extractTopLevelDomain,
  isLdhSegmentIDN, 
} from './domain-parser';
...
describe('extraction', () => {
  ...
  describe('isLdhSegmentIDN', () => {
    ...
    test('Should throw error when passed in a domain with other letters, e.g. Unicode like 黄飞鸿', () => {
      expect(() => isLdhSegmentIDN("黄飞鸿"))
      .toThrowError('Domain name not a valid LDH segment');
    });
  
    test('Should throw error when passed in a domain with dot, s.g. abc.com', () => {
      expect(() => isLdhSegmentIDN("abc.com"))
      .toThrowError('Domain name not a valid LDH segment');
    });
