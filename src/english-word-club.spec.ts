import { isGwtwc100K, isGwtwc10K, isGwtwc1K, isMIT10K } from './english-word-club';
...
describe('Gwtwc1K', () => {
  test.each(['be', 'four'] as const)('Gwtwc1K should include "%s"', (word) => {
    expect(isGwtwc1K(word)).toBeTruthy();
  });
});

describe('isSingleEnglishWord', () => {
  test.each(['be', 'four'] as const)('isSingleEnglishWord should include "%s"', (word) => {
    expect(isGwtwc1K(word)).toBeTruthy();
  });
});
